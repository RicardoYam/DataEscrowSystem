...
/**** ECALL prototypes. ****/
oe_result_t generate_keypair(
    oe_enclave_t* enclave,
    uint8_t** out_sealed_blob,
    size_t* out_sealed_size);

oe_result_t set_up_tls_server(
    oe_enclave_t* enclave,
    int* _retval,
    char* port,
    char* uuid_str);
...
oe_result_t free_tls(oe_enclave_t* enclave);

oe_result_t handle_communication_until_done(
    oe_enclave_t* enclave,
    int* _retval,
    bool keep_server_up,
    char* additional_input);

oe_result_t enclave_getPublicKey(oe_enclave_t* enclave);
