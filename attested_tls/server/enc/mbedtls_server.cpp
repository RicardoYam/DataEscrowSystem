...
int set_up_tls_server(char* server_port, bool keep_server_up)
{
    ...
    // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    unsigned char key_buf[8000]; // Ensure this buffer is large enough for the key.
    size_t key_len = 0;
    // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    if (ret != 0) {
        printf("Failed mbedtls_pk_setup: %d\n", ret);
        goto exit;
    }

    ret = mbedtls_rsa_gen_key(mbedtls_pk_rsa(pkey), mbedtls_ctr_drbg_random, &ctr_drbg, 2048, 65537);
    if (ret != 0) {
        printf("Failed mbedtls_rsa_gen_key: %d\n", ret);
        goto exit;
    }

    // Convert the private key to PEM format
    key_len = sizeof(key_buf);
    ret = mbedtls_pk_write_key_pem(pkey, key_buf, key_len);
    if (ret < 0) goto exit;

    // Convert to std::string (note: mbedtls_pk_write_key_pem guarantees null-termination)
    private_key_pem.assign((char*)key_buf);
    // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
